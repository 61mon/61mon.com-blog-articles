...
所谓的平衡之意，就是树中任意一个结点下两个子树的高度最大差别为1。（本文对于树的高度约定为：空结点高度是0，叶子结点高度是1。）
...
所谓的左左，即新插入结点在失衡结点的**左**孩子下的**左**子树中。
...
所谓的右右，即新插入结点在失衡结点的**右**孩子下的**右**子树中。
...
所谓的左右，即新插入结点在失衡结点的**左**孩子下的**右**子树中。

观察发现，"以x为根的子树"恰好符合"右右失衡"，所以先对其进行"右右旋转(rr_rotate)"，旋转之后发现"以y为根的子树"恰好符合"左左失衡"，所以再进行一次"左左旋转(ll_rotate)"。两次旋转后，恢复平衡。
...
所谓的右左，即新插入结点在失衡结点的**右**孩子下的**左**子树中。

观察发现，"以x为根的子树"恰好符合"左左失衡"，所以先对其进行"左左旋转(ll_rotate)"，旋转之后发现"以y为根的子树"恰好符合"右右失衡"，所以再进行一次"右右旋转(rr_rotate)"。两次旋转后，恢复平衡。
